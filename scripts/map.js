$(window).on("load", function () {
  ...
  function mapPoints(points, layers) {
    ...
    if (layers === undefined || layers.length === 0) {
      ...
      var pos =
        getSetting("_pointsLegendPos") == "off"
          ? "topleft"
    ...
    $("#points-legend").prepend(
      '<h6 class="pointer">' + getSetting("_pointsLegendTitle") + "</h6>"
    );
    if (getSetting("_pointsLegendIcon") != "") {
      $("#points-legend h6").prepend(
        '<span class="legend-icon"><i class="fas ' +
          getSetting("_pointsLegendIcon") +
          '"></i></span>'
      );
    }
    ...
    var columns = getSetting("_tableColumns")
      .split(",")
      ...
      function updateTable() {
        ...
        for (i in points) {
          if (
            ...
            map
              .getBounds()
              .contains(L.latLng(points[i].Latitude, points[i].Longitude))
          ) {
  ...
  function loadAllGeojsons(p) {
    if (
      p < polygonSettings.length &&
      getPolygonSetting(p, "_polygonsGeojsonURL").trim()
    ) {
  ...
  function processAllPolygons() {
    ...
    var singleMarker = L.marker([14.0583, 108.2772]);
    singleMarker.addTo(map);
    const leafletControlHeader = [];
    const leafletControlContent = [];
    var popupContent = "";
    ...
    while (
      ...
      if (divisors.length != polygonLayers.length) {
        alert(
          "Error in Polygons: The number of sets of divisors has to match the number of properties"
        );
        return;
      }
      ...
      for (i = 0; i < divisors.length; i++) {
        if (divisors[i].length == 0) {
          ...
          colors[i] = palette(
            tryPolygonSetting(p, "_colorScheme", "tol-sq"),
            divisors[i].length
          );
          ...
          alert(
            "Error in Polygons: The number of divisors should match the number of colors"
          );
          return; // Stop here
        }
      }
      ...
      for (i = 0; i < divisors.length; i++) {
        if (
          !isNaN(parseFloat(divisors[i][0].trim())) &&
          !divisors[i][0].trim().includes(":")
        ) {
          isNumerical[i] = true;
          for (j = 0; j < divisors[i].length; j++) {
      ...
      polygonsLegend.onAdd = function (map) {
        var content =
          '<h6 class="pointer">' +
          getPolygonSetting(p, "_polygonsLegendTitle") +
          "</h6>";
        content += "<form>";

        // add name of leaflet control header to array
        leafletControlHeader.push(getPolygonSetting(p, "_polygonsLegendTitle"));
        ...
        leafletControlContent.push(layer);
        content +=
          '<label><input type="radio" name="prop" value="' +
          p +
        ...
        var popup = singleMarker.bindPopup(popupContent);
        popup.addTo(map);
    ...
    let baseLayers = {};
    // code right
    leafletControlHeader.forEach((item, i) => {
      baseLayers[item] = allGeojsons[i];
    });

    let baseTree = leafletControlHeader.map(function (label, i) {
      return {
        label: label,
        children: [
          {
            label: leafletControlContent[i],
            layer: allGeojsons[i],
            name: label,
          },
          {
    ...
    let leafletControlLayerSelectors = [];
    elements.forEach((item, index) => {
      if (item.hasAttribute("name")) {
        leafletControlLayerSelectors.push(item);
      }
    });
    leafletControlLayerSelectors.forEach((item, index) => {
      polygon = index < 7 ? index : 6;
      layer = 0;
      if (layer == -1) {
        $(".polygons-legend" + polygon)
          .find(".polygons-legend-scale")
  ...
  function updatePolygons() {
    ...
    if (allDivisors[p][z] == "") {
      $(".polygons-legend" + p)
        .find(".polygons-legend-scale")
        .css({ margin: "0px", padding: "0px", border: "0px solid" });
      return;
    }
    ...
    for (var i = 0; i < allDivisors[p][z].length; i++) {
      ...
      labels.push(
        '<i style="background:' +
          color +
          "; opacity: " +
          tryPolygonSetting(p, "_colorOpacity", "0.7") +
          '"></i> ' +
          from +
          (to && isNum ? " &ndash; " + to : isNum ? "+" : "")
      );
    }
  ...
  function onEachFeature(feature, layer) {
    ...
    if (
      getPolygonSetting(polygon, "_popupProp") == "" &&
      getPolygonSetting(polygon, "_polygonDisplayImages") == "off"
    ) {
      return;
    }
    ...
    for (i in props) {
      ...
      info += props[i][1] ? props[i][1].trim() : props[i][0].trim();
    ...
    if (getPolygonSetting(polygon, "_polygonLabel") !== "") {
      var myTextLabel = L.marker(
        polylabel(layer.feature.geometry.coordinates, 1.0).reverse(),
        {
          icon: L.divIcon({
            className: "polygon-label" + polygon + " polygon-label",
            html: feature.properties[
              getPolygonSetting(polygon, "_polygonLabel")
            ],
          }),
        }
      );
  ...
  function onMapDataLoad(options, points, polylines) {
    ...
    // Add polygons
    if (
      getPolygonSetting(0, "_polygonsGeojsonURL") &&
    ...
    // Add toggle control button
    if (getSetting("_mapToggleLegend") !== "off") {
      var toggleControlButton = L.Control.extend({
        options: {
          position: getSetting("_mapToggleLegend"),
        },
        onAdd: function () {
          var button = L.DomUtil.create("button", "toggle-control-button");
          button.innerHTML = "Toggle Legend";
          button.onclick = function () {
            // Toggle the visibility of the legend here
            var controls = document.querySelectorAll(".ladder");
      ...
      // Add the toggle control button to the map
      map.addControl(new toggleControlButton());
    }
    ...
    function showMap() {
      if (completePoints && completePolylines && completePolygons) {
        $(".ladder h6").append(
          '<span class="legend-arrow"><i class="fas fa-chevron-down"></i></span>'
        );
        ...
        for (i in allPolygonLegends) {
          if (getPolygonSetting(i, "_polygonsLegendIcon") != "") {
            $(".polygons-legend" + i + " h6").prepend(
              '<span class="legend-icon"><i class="fas ' +
                getPolygonSetting(i, "_polygonsLegendIcon") +
                '"></i></span>'
            );
          }
        }
  ...
  function addTitle() {
    ...
    if (dispTitle !== "off") {
      ...
      if (dispTitle == "topleft") {
        $("div.leaflet-top").prepend(
          '<div class="map-title leaflet-bar leaflet-control leaflet-control-custom">' +
            title +
            subtitle +
            "</div>"
        );
  ...
  function processPolylines(p) {
    ...
    var pos =
      getSetting("_polylinesLegendPos") == "off"
        ? "topleft"
      ...
      $.getJSON(
        ...
        (function (index) {
          return function (data) {
            ...
            polylinesLegend.addOverlay(
              line,
              '<i class="color-line" style="background-color:' +
                p[index]["Color"] +
                '"></i> ' +
                p[index]["Display Name"]
            );
            ...
            if (index == 0) {
              ...
              if (getSetting("_polylinesLegendTitle") != "") {
                $("#polylines-legend").prepend(
                  '<h6 class="pointer">' +
                    getSetting("_polylinesLegendTitle") +
                    "</h6>"
                );
                if (getSetting("_polylinesLegendIcon") != "") {
                  $("#polylines-legend h6").prepend(
                    '<span class="legend-icon"><i class="fas ' +
                      getSetting("_polylinesLegendIcon") +
                      '"></i></span>'
                  );
                }
  ...
  function togglePolygonLabels() {
    for (i in allTextLabels) {
      if (map.getZoom() <= tryPolygonSetting(i, "_polygonLabelZoomLevel", 9)) {
        ...
        if (
          $(".polygons-legend" + i + " input[name=prop]:checked").val() != "-1"
        ) {
  ...
  function addBaseMap() {
    ...
    if (trySetting("_displayHSTSLayer", "on") === "on") {
      var HSTSGeoJsonURL = trySetting(
        "_HSTSGeoJsonURL",
        "https://raw.githubusercontent.com/ccrivietnam/ccriqtstcmh/main/geodata/hoangsa_truongsa.geojson"
      );
      // Check if HSTSGeoJsonURL is a valid url, if not get raw content from github repo
      if (!isValidURL(HSTSGeoJsonURL)) {
        HSTSGeoJsonURL =
          "https://raw.githubusercontent.com/ccrivietnam/ccriqtstcmh/main/" +
          HSTSGeoJsonURL;
      }
  ...
  $.ajax({
    ...
    error: function () {
      ...
      if (typeof googleApiKey !== "undefined" && googleApiKey) {
        ...
        var spreadsheetId =
          googleDocURL.indexOf("/d/") > 0
            ? googleDocURL.split("/d/")[1].split("/")[0]
            : googleDocURL;

        $.getJSON(apiUrl + spreadsheetId + "?key=" + googleApiKey).then(
          function (data) {
            var sheets = data.sheets.map(function (o) {
